...
from src.network import TwinnedStateActionFunction, GaussianPolicy
from src.utils import disable_gradients, soft_update, update_params, \
    assert_action
...
class SAC(Algorithm):
    ...
    def __init__(self, state_dim, action_dim, device, gamma=0.99, nstep=1,
        ...
        # Optimizers.
        self._policy_optim = PCGrad(Adam(self._policy_net.parameters(), lr=policy_lr))
        self._q_optim = PCGrad(Adam(self._online_q_net.parameters(), lr=q_lr))
        ...
        self._alpha_optim = PCGrad(Adam([self._log_alpha], lr=entropy_lr))
